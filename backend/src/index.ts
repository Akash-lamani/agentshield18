...
import { renderHtmlReport, writeScanResultsJson } from "./reporter/html.js";
import { runOpusPipeline, renderOpusAnalysis } from "./opus/index.js";
import { runGSKPipeline } from "./gsk/render.js";
...
program
  ...
  .option("--report <format>", "Alias for --format (html, json, terminal, markdown)")
  ...
  .option("--gsk", "Enable GSK (Gemini) three-agent deep analysis (requires GSK_API_KEY)", false)
  .option("--grok", "Alias for --gsk (deprecated, use --gsk)", false)
  ...
  .option("--framework <framework>", "Force framework: langchain, openai, crewai, claude, auto (default: auto)", "auto")
  ...
  .action(async (options) => {
    ...
    const result = scan(targetPath, options.framework ?? "auto");
    ...
    // --report is an alias for --format; --report takes precedence
    const effectiveFormat: string = (options.report as string | undefined) ?? (options.format as string) ?? "terminal";
    const outputPath: string | undefined = options.output as string | undefined;

    // Output static scan
    let reportContent = "";
    switch (effectiveFormat) {
      case "json":
        reportContent = renderJsonReport(report);
        break;
      case "markdown":
        reportContent = renderMarkdownReport(report);
        break;
      case "html": {
        reportContent = renderHtmlReport(report);
        // Also write scan-results.json for the dashboard to consume
        const htmlOutPath = outputPath ?? "report.html";
        try {
          writeScanResultsJson(report, htmlOutPath);
        } catch {
          // non-fatal — dashboard json is best-effort
        }
        break;
      }
      default:
        reportContent = renderTerminalReport(report);
    }

    if (outputPath) {
      const { dirname: dn } = await import("node:path");
      const dir = dn(resolve(outputPath));
      if (dir && dir !== ".") {
    ...
    // ── Phase 6b: GSK (Gemini) three-agent deep analysis ────
    if (options.gsk || options.grok) {
      if (!process.env.GSK_API_KEY) {
        console.error(
          "\nError: GSK_API_KEY environment variable required for --gsk mode.\n" +
            "Get your Google AI Studio key at https://aistudio.google.com/apikey and set it with:\n" +
            "  export GSK_API_KEY=your-key-here\n"
        );
        process.exit(1);
      }
      logger.log({ level: "info", phase: "gsk", message: "Running GSK pipeline" });
      try {
        await runGSKPipeline(report);
        logger.log({ level: "info", phase: "gsk", message: "GSK analysis complete" });
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        console.error(`\nGSK analysis failed: ${message}`);
        console.error("The static scan results above are still valid.\n");
        logger.log({ level: "error", phase: "gsk", message: `GSK failed: ${message}` });
      }
    }
...
policyCmd
  .command("list")
  .description("List all fields and defaults of the organization policy schema")
  .action(async () => {
    console.log("\n  AgentShield Policy Schema\n");
    console.log("  Fields available in a policy file (.agentshield/policy.json):\n");

    const fields = [
      { name: "version",           type: "number",   default: "1 (required)", desc: "Schema version — must be 1" },
      { name: "name",              type: "string",   default: "optional",     desc: "Human-readable policy name" },
      { name: "description",       type: "string",   default: "optional",     desc: "Policy description" },
      { name: "min_score",         type: "number",   default: "60",           desc: "Minimum acceptable security score (0-100)" },
      { name: "max_severity",      type: "enum",     default: "critical",     desc: "Max allowed finding severity: critical|high|medium|low|info" },
      { name: "required_deny_list", type: "string[]", default: "[]",          desc: "Strings that MUST appear in permissions.deny" },
      { name: "banned_mcp_servers", type: "string[]", default: "[]",         desc: "MCP server names that are banned" },
      { name: "banned_tools",      type: "string[]", default: "[]",           desc: "Tools that must NOT appear in allow list" },
      { name: "required_hooks",    type: "object[]", default: "[]",           desc: "Hook patterns required in settings (event, pattern, description)" },
    ];

    for (const f of fields) {
      console.log(`  ${f.name.padEnd(22)} ${f.type.padEnd(12)} default: ${f.default}`);
      console.log(`  ${"".padEnd(22)} ${f.desc}\n`);
    }

    console.log("  Example: agentshield policy init\n");
  });

policyCmd
  .command("validate")
  .description("Validate a policy file's syntax and schema")
  .argument("<path>", "Path to the policy file")
  .action(async (policyFilePath: string) => {
    const absPath = resolve(policyFilePath);

    if (!existsSync(absPath)) {
      console.error(`\n  Error: Policy file not found: ${absPath}\n`);
      process.exit(1);
    }

    try {
      const { loadPolicy } = await import("./policy/index.js");
      const result = loadPolicy(absPath);

      if (!result.success) {
        console.error(`\n  Policy validation FAILED\n`);
        console.error(`  File:  ${absPath}`);
      ...
      if (policy.name) console.log(`  Name:        ${policy.name}`);
      if (policy.description) console.log(`  Description: ${policy.description}`);
      console.log(`  Min score:   ${policy.min_score}`);
      console.log(`  Max severity: ${policy.max_severity}`);
      console.log(`  Required deny rules: ${policy.required_deny_list.length}`);
      console.log(`  Banned MCP servers:  ${policy.banned_mcp_servers.length}`);
      console.log(`  Required hooks:      ${policy.required_hooks.length}\n`);
    } catch (e) {
      console.error(`\n  Failed to validate policy: ${(e as Error).message}\n`);
      process.exit(1);
    }
  });
...
policyCmd
  .command("apply")
  .description("Apply a policy file to a target path (scan + enforce)")
  .argument("<policy-path>", "Path to the policy file")
  ...
  .action(async (policyFilePath: string, options: { path: string; report: string; output?: string }) => {
    const absPolicy = resolve(policyFilePath);
    const targetPath = resolveTargetPath(options.path);

    if (!existsSync(absPolicy)) {
      console.error(`\n  Error: Policy file not found: ${absPolicy}\n`);
      process.exit(1);
    }
    ...
    console.log(`\n  Applying policy: ${absPolicy}`);
    console.log(`  Target:         ${targetPath}\n`);

    const result = scan(targetPath, "auto");
    const report = calculateScore(result);

    const { loadPolicy, evaluatePolicy, renderPolicyEvaluation } = await import("./policy/index.js");
    const policyResult = loadPolicy(absPolicy);

    if (!policyResult.success) {
      console.error(`  Error loading policy: ${policyResult.error}\n`);
      process.exit(1);
    }

    const evaluation = evaluatePolicy(policyResult.policy, result.findings, report.score, result.target?.files ?? []);


    // Output scan report
    let reportContent = "";
    switch (options.report) {
      case "json": reportContent = renderJsonReport(report); break;
      case "html": {
        reportContent = renderHtmlReport(report);
        if (options.output) {
          try { writeScanResultsJson(report, options.output); } catch { /* ignore */ }
        }
        break;
      }
      default: reportContent = renderTerminalReport(report);
    }

    if (options.output) {
      const { mkdirSync: mkd } = await import("node:fs");
      const { dirname: dn } = await import("node:path");
      const dir = dn(resolve(options.output));
      if (dir && dir !== ".") {
        try { mkd(dir, { recursive: true }); } catch { /* ignore */ }
      }
