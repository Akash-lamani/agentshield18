...
import type { ConfigFile, ConfigFileType, DetectedFramework, ScanTarget } from "../types.js";
...
export function discoverConfigFiles(rootPath: string): ScanTarget {
  ...
  const framework = detectFramework(rootPath, files);

  return { path: rootPath, files, framework };
}

/**
 * Detect which AI agent framework is in use by inspecting files on disk
 * and the already-collected config files.
 */
export function detectFramework(
  rootPath: string,
  configFiles: ReadonlyArray<ConfigFile>
): DetectedFramework {
  // Check config files for Claude-specific markers first
  const hasClaudeFiles = configFiles.some(
    (f) =>
      f.type === "claude-md" ||
      f.type === "settings-json" ||
      f.type === "mcp-json" ||
      f.type === "agent-md"
  );
  if (hasClaudeFiles) return "claude";

  // Walk top-level Python files for framework imports
  const pythonFiles = collectPythonFiles(rootPath);

  for (const pyPath of pythonFiles) {
    try {
      const content = readFileSync(pyPath, "utf-8");
      if (content.includes("from crewai") || content.includes("import crewai")) {
        return "crewai";
      }
      if (
  ...
  // Check for OpenAI JSON configs
  const jsonFiles = collectJsonFiles(rootPath);
  for (const jsonPath of jsonFiles) {
    const name = basename(jsonPath).toLowerCase();
    if (name === "assistant.json" || name === "openai-config.json") {
      return "openai";
    }
    try {
  ...
  // Check for .env with OpenAI/Anthropic keys (LangChain style)
...
function collectPythonFiles(rootPath: string): ReadonlyArray<string> {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory() && results.length < 50) {
        // One level deep only to keep discovery fast
        try {
          const subEntries = readdirSync(join(rootPath, entry.name), { withFileTypes: true });
          for (const sub of subEntries) {
            if (sub.isFile() && sub.name.endsWith(".py")) {
              results.push(join(rootPath, entry.name, sub.name));
            }
          }
        } catch {
          // unreadable dir
...
function collectJsonFiles(rootPath: string): ReadonlyArray<string> {
  const results: string[] = [];
  try {
    const entries = readdirSync(rootPath, { withFileTypes: true });
    for (const entry of entries) {
