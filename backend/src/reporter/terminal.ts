...
export function renderTerminalReport(report: SecurityReport): string {
  ...
  const tableRows: Array<[string, number, string]> = [
    ["Critical", s.critical, chalk.red.bold(`${s.critical}`)],
    ["High",     s.high,     chalk.yellow.bold(`${s.high}`)],
    ["Medium",   s.medium,   chalk.blue(`${s.medium}`)],
    ["Low",      s.low,      chalk.dim(`${s.low}`)],
    ["Info",     s.info,     chalk.dim(`${s.info}`)],
  ];
  lines.push(chalk.dim("  ┌─────────────┬─────────┐"));
  lines.push(chalk.dim("  │ Severity    │ Count   │"));
  lines.push(chalk.dim("  ├─────────────┼─────────┤"));
  for (const [label, count, colored] of tableRows) {
    const labelPad = label.padEnd(11);
    const countPad = colored.padEnd(7 + (colored.length - String(count).length));
    lines.push(`  │ ${chalk.dim(labelPad)} │ ${countPad} │`);
  }
  lines.push(chalk.dim("  ├─────────────┼─────────┤"));
  lines.push(`  │ ${chalk.dim("Total".padEnd(11))} │ ${chalk.bold(String(s.totalFindings).padEnd(7))} │`);
  lines.push(chalk.dim("  └─────────────┴─────────┘"));
  lines.push("");
  lines.push(chalk.dim(`  Files scanned: ${s.filesScanned}`));
  if (s.autoFixable > 0) {
    lines.push(chalk.green(`  Auto-fixable:  ${s.autoFixable} (run with --fix)`));
  }
  lines.push("");
